...
export function welcome(width = termWidth()) {
    ...
    const art = width < 50 ? "" : ART.map((l) => centerLine(paintArt(l), width)).join("\n");
...
async function localModelsMenu() {
    ...
    if (picked === 0) {
        ...
        if (ol === null) {
            ...
            console.log(`[harness] local models: ${ol.runners.length} resident ~${fmtMem(ol.totalKb)}${names ? ` (${names})` : ""}`);
        }
    }
    else if (picked === 1) {
